...
if ( 'speechSynthesis' in window )
{
	...
	function pausing( e ) {
		progress_char += e.utterance.text.charAt( e.charIndex );
	}

	function speechButtonAction( e ){
		var $el = e.target;
		
		while ( $el && $el.nodeName.toLowerCase() != 'button' )
		{
			$el = $el.parentNode;
		}
	...
	var speech = new SpeechSynthesisUtterance(),
		...
		$skip = $content.querySelectorAll('.anchorable__anchor, aside, .dont_read'),
		$controls = document.createElement('div'),
		$intro = document.createElement('p'),
		$button = document.createElement('button'),
		$buttons = $intro.cloneNode(true),
		$play,
		$pause,
		$faster,
		$slower,
		$speed = $intro.cloneNode(true),
		$speed_value;
	...
	$controls.addEventListener( 'click', speechButtonAction, false );
	...
	//$faster = $button.cloneNode('true');
	//$faster.classList.add('media-controls__button--faster');
	//$faster.setAttribute( 'aria-label', 'Faster' );
	//$buttons.appendChild($faster);
	//$slower = $button.cloneNode('true');
	//$slower.classList.add('media-controls__button--slower');
	//$slower.setAttribute( 'aria-label', 'Slower' );
	//$buttons.appendChild($slower);
	$controls.appendChild($buttons);

	// Speed
	//$speed.innerText = 'Reading Speed: ';
	//$speed_value = document.createElement('output');
	//$speed_value.value = rate + '×';
